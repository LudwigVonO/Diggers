...
const PgMutationUpsertPlugin = builder => {
    builder.hook('GraphQLObjectType:fields', (fields, build, context) => {
        const { extend, gql2pg, graphql: { GraphQLObjectType, GraphQLInputObjectType, GraphQLNonNull, GraphQLString }, inflection, newWithHooks, parseResolveInfo, pgGetGqlInputTypeByTypeIdAndModifier, pgGetGqlTypeByTypeIdAndModifier, pgIntrospectionResultsByKind, pgQueryFromResolveData: queryFromResolveData, pgSql: sql, pgViaTemporaryTable: viaTemporaryTable, pgOmit: omit } = build;
            ...
            memo[fieldName] = fieldWithHooks(fieldName, context => {
                ...
                return {
                    ...
                    async resolve(data, { where, input }, { pgClient }, resolveInfo) {
                        ...
                        const uniqueKeys = uniqueConstraints.reduce((acc, constraint) => {
                            const keys = constraint.keyAttributeNums.map(num => attributes.find(attr => attr.num === num));
                            return [...acc, ...keys];
                        }, []);
                        ...
                        insert into ${sql.identifier(table.namespace.name, table.name)} ${sqlColumns.length
                            ? sql.fragment `(
                            ${sql.join(sqlColumns, ', ')}
                          ) values(${sql.join(sqlValues, ', ')})
                          on conflict (${sql.join(uniqueKeyColumns, ',  ')})  do update
